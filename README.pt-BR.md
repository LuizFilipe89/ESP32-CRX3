...
Cada rede falsa recebe **seu próprio canal fixo de 2,4GHz** (distribuídos igualmente entre os 13 canais), igual a um ponto de acesso de verdade — e o ESP32 fica parado em cada canal por vez (~1 segundo cada), retransmitindo densamente o lote inteiro de redes falsas daquele canal durante todo o tempo em que está ali, antes de passar pro próximo. Fixar cada rede falsa num canal permanente (em vez de deixar o canal de transmissão mudar com o tempo) é o que mantém ela aparecendo de forma consistente na lista de Wi-Fi do celular, em vez de piscar aparecendo e sumindo — a maioria dos celulares trata uma rede que parece "pular" de canal como instável e a esconde. Uma checagem de segurança embutida monitora o log serial em busca de qualquer lote que comece a consumir tempo demais do seu orçamento, então isso se mantém confiável ao longo de minutos de execução contínua, não só numa rajada rápida. Como o rádio só pode estar em um canal por vez, esse ataque derruba o AP de gerência enquanto roda, assim como o Deauth e o Evil Twin.
...
Entra numa rede Wi-Fi escolhida como estação, escaneia a subrede local por hosts com a porta de impressão bruta aberta (9100 / JetDirect), e pode disparar um trabalho de impressão bruto pra uma ou mais impressoras encontradas. A aba já está presente e funcional na interface, mas esse recurso ainda está sendo refinado — espere arestas soltas se for testar.
