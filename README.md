...
Every fake network gets **its own fixed 2.4GHz channel** (spread evenly across all 13), just like a real access point — and the ESP32 dwells on each channel in turn (~1 second each), densely re-broadcasting that channel's whole batch of fake APs the entire time it's parked there, before moving to the next. Pinning each fake AP to one permanent channel (rather than having its broadcast channel drift over time) is what keeps it showing up consistently in a phone's Wi-Fi list instead of flickering in and out — most phones treat a network that appears to "jump" channels as unstable and hide it. A built-in self-monitoring check watches the serial log for any batch that starts eating too much of its time budget, so this stays reliable over minutes of continuous running, not just a quick burst. Since the radio can only be on one channel at a time, this attack takes the management AP down while it runs, same as Deauth and Evil Twin.
