...
/* Deliberately stays on whatever channel the radio is already parked on
 * (normally the management AP's channel) instead of hopping. A channel-hop
 * version was tried and made real-world visibility worse: it had to take
 * the management AP down to free the radio, so the phone lost its
 * connection and fell back to slow/throttled background scanning — versus
 * staying anchored on the AP's channel, where the phone picks up beacons
 * passively without needing to actively rescan at all. Single-channel with
 * the AP still up is the version that reliably worked when tested. */
