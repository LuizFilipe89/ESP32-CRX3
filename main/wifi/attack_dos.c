...
void attack_dos_stop() {
    switch(method){
        case ATTACK_DOS_METHOD_ROGUE_AP:
            /* esp_wifi_set_mac() requires the AP interface disabled — stop it,
             * restore the original MAC, THEN re-enable with normal mgmt config.
             * (Restoring the MAC after re-enabling the AP would abort/reboot
             * the chip, same as the start-side bug this mirrors.) */
            wifictl_mgmt_ap_stop();
